...
function wp_bootstrap_starter_custom_scripts(){
	//file name would need to always have the full directory name
	$custom_css_folder = scandir('C:\wamp64\www\wptest\wp-content\themes\wp-bootstrap-starter\inc\assets\css\page-specific');
	foreach($custom_css_folder as $file){
		$page_name = strstr((string)$file,'.css', true);
		if(is_page($page_name)){
			wp_register_style($file,'C:\wamp64\www\wptest\wp-content\themes\wp-bootstrap-starter\inc\assets\css\page-specific/' + $file);
			wp_enqueue_style($page_name,get_stylesheet_directory_uri().'/inc/assets/css/page-specific/'.$file);
		}
	}
}
